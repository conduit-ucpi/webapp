...
describe('Currency Display Integration - No Double Conversion', () => {
  ...
  describe('Verify proper currency parameter handling', () => {
    it('should treat USDC parameter as already-converted USDC amounts', () => {
      ...
      expect(result).toBe('$1000000.00'); // 1000000 USDC displayed as $1,000,000
    });
    ...
    it('should handle small USDC amounts correctly', () => {
      ...
      expect(result).toBe('$1.00'); // 1 USDC displayed as $1.00
    });
  });

  describe('Edge cases that could reveal double conversion', () => {
    it('should handle zero correctly', () => {
      expect(displayCurrency(0, 'microUSDC')).toBe('$0.00');
    });

    it('should handle very small amounts correctly', () => {
      expect(displayCurrency(1, 'microUSDC')).toBe('$0.00'); // 1 microUSDC = 0.000001 USDC ≈ $0.00
    });

    it('should handle very large amounts correctly', () => {
      expect(displayCurrency(1000000000, 'microUSDC')).toBe('$1000.00'); // 1 billion microUSDC = $1000
    });
  });
  ...
  describe('Real-world contract amounts', () => {
    ...
    const testCases = [
      { microUSDC: 100000, expectedDisplay: '$0.10', description: '10 cents' },
      { microUSDC: 500000, expectedDisplay: '$0.50', description: '50 cents' },
      { microUSDC: 1000000, expectedDisplay: '$1.00', description: '1 dollar' },
      { microUSDC: 2500000, expectedDisplay: '$2.50', description: '2.50 dollars' },
      { microUSDC: 10000000, expectedDisplay: '$10.00', description: '10 dollars' },
      { microUSDC: 50000000, expectedDisplay: '$50.00', description: '50 dollars' },
      { microUSDC: 100000000, expectedDisplay: '$100.00', description: '100 dollars' },
    ];
  ...
  describe('Detect double conversion errors', () => {
    it('should NOT show $0.001 as $0.000001 (double conversion error)', () => {
      ...
      expect(result).toBe('$0.001');
      expect(result).not.toBe('$0.00'); // Would be double conversion error
    });
    ...
    it('should NOT show $1000 as $1000000 (reverse double conversion error)', () => {
      ...
      expect(result).not.toBe('$1000000.00'); // Would be conversion error
    });
  });
});