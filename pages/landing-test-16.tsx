...
/*
 * Landing test 16 — 11, tightened.
 * The right column is only ever titles; the detail lives on the left and follows the
 * pointer, so nothing on the right ever grows or shifts. The nav's account link is
 * "My dashboard" and the hero drops the "already using?" line for it. Pricing is a
 * side-by-side table: what a processor charges for each line, against ours.
 */

const ID = 'lt-16';

export default function LandingTest16() {
  const [siteName, setSiteName] = useState('Stabledrop.me');
  useEffect(() => setSiteName(getSiteNameFromDomain()), []);

  return (
    <>
      <SEO title={SEO_TITLE_GETPAID} description={SEO_DESCRIPTION_GETPAID} keywords={SEO_KEYWORDS} canonical="/landing-test-16" structuredData={[financialServiceSchema, articleSchema]} />
      <Head>
