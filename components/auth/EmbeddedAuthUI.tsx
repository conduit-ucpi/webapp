...
export default function EmbeddedAuthUI({ className = '', onSuccess, compact = false }: EmbeddedAuthUIProps) {
  ...
  const handleConnect = async (method: string) => {
    try {
      ...
      if (method === 'google' || method === 'facebook' || method === 'email') {
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-3">
          <Button
            onClick={() => handleConnect('metamask')}
            disabled={isAnyLoading}
            className="w-full bg-[#F6851B] hover:bg-[#E2761B] text-white flex items-center justify-center"
          >
