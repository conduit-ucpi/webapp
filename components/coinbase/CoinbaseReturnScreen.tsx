...
interface CoinbaseReturnScreenProps {
  /** Posted to the opener so the page that launched Coinbase knows we are back. */
  message: string;
}
 ...
 *
 * It has to serve two shapes, because the widget is opened differently per
 * device. On desktop it runs inside the popup: it tells the opener and closes,
 * so the user is returned to what they were already looking at. On mobile the
 * whole tab went to Coinbase, so it forwards back to the page they left.
 *
 ...
 */
export default function CoinbaseReturnScreen({ message }: CoinbaseReturnScreenProps) {
  ...
  }, [router.isReady, router.query.return, router, message]);
