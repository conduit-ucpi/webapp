...
/*
 * Landing test 15 — "questions, answered as you scroll".
 * Every chapter is a question a visitor would actually ask, and the right column is the
 * list of answers as single rows — one line each, folded, opening on hover. The left
 * panel repeats the row under the pointer. It reads like a conversation, not a brochure.
 */

const ID = 'lt-15';

export default function LandingTest15() {
  const [siteName, setSiteName] = useState('Stabledrop.me');
  useEffect(() => setSiteName(getSiteNameFromDomain()), []);

  return (
    <>
      <SEO title={SEO_TITLE_GETPAID} description={SEO_DESCRIPTION_GETPAID} keywords={SEO_KEYWORDS} canonical="/landing-test-15" structuredData={[financialServiceSchema, articleSchema]} />
      <Head>
      ...
      <div id={ID} className={`${lt.bg} ${lt.fg}`} style={{ fontFamily: 'var(--lt-font)' }}>
        ...
        <ScrollyHero
          ...
          headline={<>Get paid. <span className={lt.accentText}>Every question you&apos;d ask, answered as you scroll.</span></>}
          sub="Start with the one that matters: the money waits in escrow until the payout date, the buyer is protected, and you cannot be charged back. The rest is below, one line at a time."
        ...
        <HoverChapter
          label="Question one"
          title="What actually happens to the money?"
          ariaLabel="How it works"
          fallback="Three steps. Hover a row for the detail."
        ...
        <Chapter
          id="demo"
          label="Question two"
          title="What does my customer see?"
        ...
        >
          <p className={lt.muted}>This. The actual checkout, for a real $0.001 payment; and under it, what a payment of any size costs and returns.</p>
          <DemoButtons className="mt-6" />
        </Chapter>

        <HoverChapter
          label="Question three"
          title="What does a processor take that you don't?"
          ariaLabel="For merchants"
          fallback="Six line items. Hover any of them."
        ...
        <HoverChapter
          label="Question four"
          title="Why would a buyer trust this?"
          ariaLabel="For buyers"
          fallback="Because the contract, not the seller and not us, holds the money."
          items={BUYER_POINTS}
          columns={1}
          reveal
          compact
        ...
        <HoverChapter
          label="Question five"
          title="Can an AI agent pay with it?"
        ...
        <Chapter label="Question six" title="How do I put it on my site?" ariaLabel="Embed in your platform" aside={<EmbedExamples layout="side" />}>
          <p className={lt.muted}>A plugin for WordPress and Shopify, one script tag for everything else, and one URL for any AI agent. Ten minutes.</p>
          <div className="mt-6">
            <Link href="/integrate" className={lt.btnSecondary}>Read the SDK guide</Link>
          </div>
        </Chapter>

        <Chapter label="Question seven" title="Is it for someone like me?" ariaLabel="Who it is for" aside={<AudienceTabs columns={2} />}>
          <p className={lt.muted}>Pick who you are and see how people like you use it.</p>
        </Chapter>

        <HoverChapter
          label="Question eight"
          title="What else do people ask first?"
          ariaLabel="Questions"
          fallback="Wallets, refunds, custody, cost, early payment. Hover a row."
          items={FAQ_SHORT.map((f) => ({ label: f.q, text: f.a }))}
          columns={1}
          reveal
          compact
          actions={<Link href="/faq" className={lt.btnSecondary}>All questions</Link>}
        />
