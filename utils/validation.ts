 ...
 */
export function getStatusDisplay(status: string | undefined, isBuyer: boolean = false, isSeller: boolean = false): StatusDisplay {
...
/**
 * Single source of truth for determining the primary action button on contract cards
 * Used by EnhancedContractCard to show the main action button
 */
export function getPrimaryAction(
  ...
  // Pending contracts - buyer can accept
  if (isPending && isBuyer && !isExpired && contractState === 'OK') {
    return {
  ...
  if (contractStatus === 'ACTIVE') {
    // Expired active contracts - seller can claim
    ...
    // Non-expired active contracts - buyer can dispute
